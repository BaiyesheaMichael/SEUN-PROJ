...
def execute_csv( *event ):     
    # print( df )
    # def execute_csv( df, pd, np, date, datetime, time, t1, plot_number_var ):     
...
##    print(df['DATE OF POSSESSION'].dtype)
    ...
    df['PLOT NUMBER'] = df['PLOT NUMBER'].astype(str)
    # df['NAME'] = df['NAME'].str.lower()
    # df['NAME'] = df['PLOT NUMBER'].lower()
    plot_num = plot_number_var.get()
    search_result = df[ np.logical_or( df[ 'PLOT NUMBER' ] == plot_num, df[ 'NAME' ].str.lower() == plot_num.lower() ) ]

    df.set_index('PLOT NUMBER', inplace=True, drop=False), #ignore the cell's value is Nancase=False)] 
    x = df[ 'NAME' ].str.lower()
    if plot_num not in df.values:
        search_result = df[ np.logical_or( df[ 'PLOT NUMBER' ].str.contains( plot_num ), x.str.contains( plot_num.lower() ) ) ]
...
sheets = ["VINCENT%20AZIKE%20STREET"]
...
for sheet in sheets:
   ...
   # print(df.to_string())
    ...
    # e1.bind( "<Return>", execute_csv( df, pd, np, date, datetime, time, t1, plot_number_var ) ) 
    ...
    # execute_csv( df, pd, np, date, datetime, time, t1, plot_number_var )  
